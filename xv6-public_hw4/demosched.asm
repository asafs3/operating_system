...
int main (int argc, char *argv[]) {
  ...
  13:	83 ec 14             	sub    $0x14,%esp
    setprio(7);
  16:	6a 07                	push   $0x7
  18:	e8 2e 04 00 00       	call   44b <setprio>
  1d:	83 c4 10             	add    $0x10,%esp
  20:	eb 10                	jmp    32 <main+0x32>
  ...
  28:	74 4d                	je     77 <main+0x77>
    for (int i = 0 ; i < childrenNum ; i++ ) {
  2a:	83 c3 01             	add    $0x1,%ebx
  2d:	83 fb 10             	cmp    $0x10,%ebx
  30:	74 2e                	je     60 <main+0x60>
        pid = fork();
  32:	e8 54 03 00 00       	call   38b <fork>
        if (pid < 0 ) {
  37:	85 c0                	test   %eax,%eax
  39:	79 ed                	jns    28 <main+0x28>
            printf(1, "failed to create fork for process index %d\n", i);
  ...
  4e:	83 c4 10             	add    $0x10,%esp
    for (int i = 0 ; i < childrenNum ; i++ ) {
  51:	83 fb 10             	cmp    $0x10,%ebx
  54:	75 dc                	jne    32 <main+0x32>
  56:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  5d:	8d 76 00             	lea    0x0(%esi),%esi
  60:	bb 10 00 00 00       	mov    $0x10,%ebx
  65:	8d 76 00             	lea    0x0(%esi),%esi
    ...
    for (int k = 0 ; k < childrenNum ; k++ ) {
        wait();
  68:	e8 2e 03 00 00       	call   39b <wait>
    for (int k = 0 ; k < childrenNum ; k++ ) {
...
int func(){
  ...
  d3:	53                   	push   %ebx
        sum = sum/ 10000;
  d4:	bb ad 8b db 68       	mov    $0x68db8bad,%ebx
int func(){
  d9:	83 ec 0c             	sub    $0xc,%esp
    int sum = 0;
  dc:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
  e3:	d9 7d f6             	fnstcw -0xa(%ebp)
  e6:	0f b7 45 f6          	movzwl -0xa(%ebp),%eax
  ea:	80 cc 0c             	or     $0xc,%ah
  ed:	66 89 45 f4          	mov    %ax,-0xc(%ebp)
            sum = sum + pieCalc;   // useless calculations to consume CPU time
  f1:	dd 05 c8 08 00 00    	fldl   0x8c8
int func(){
  f7:	b8 e3 b5 4f 03       	mov    $0x34fb5e3,%eax
  fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
            sum = sum + pieCalc;   // useless calculations to consume CPU time
 100:	db 45 f0             	fildl  -0x10(%ebp)
 103:	d8 c1                	fadd   %st(1),%st
 105:	d9 6d f4             	fldcw  -0xc(%ebp)
 108:	db 5d f0             	fistpl -0x10(%ebp)
 10b:	d9 6d f6             	fldcw  -0xa(%ebp)
        for (int z = 0; z < 55555555; z++) {
 10e:	83 e8 01             	sub    $0x1,%eax
 111:	75 ed                	jne    100 <func+0x30>
 113:	dd d8                	fstp   %st(0)
        sum = sum - 388888880;
 115:	8b 4d f0             	mov    -0x10(%ebp),%ecx
 118:	81 e9 30 f9 2d 17    	sub    $0x172df930,%ecx
        sum = sum/ 10000;
 11e:	89 c8                	mov    %ecx,%eax
 120:	c1 f9 1f             	sar    $0x1f,%ecx
 123:	f7 eb                	imul   %ebx
 125:	c1 fa 0c             	sar    $0xc,%edx
 128:	29 ca                	sub    %ecx,%edx
 12a:	89 55 f0             	mov    %edx,-0x10(%ebp)
    while (sum != 55555) {
 12d:	81 fa 03 d9 00 00    	cmp    $0xd903,%edx
 133:	75 bc                	jne    f1 <func+0x21>
}
 135:	8b 5d fc             	mov    -0x4(%ebp),%ebx
 138:	b8 03 d9 00 00       	mov    $0xd903,%eax
 13d:	c9                   	leave  
 13e:	c3                   	ret    
 13f:	90                   	nop
 ...
 46c:	89 d1                	mov    %edx,%ecx
{
 ...
 499:	0f b6 92 30 09 00 00 	movzbl 0x930(%edx),%edx
 ...
 519:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
 ...
 589:	ff 24 85 d8 08 00 00 	jmp    *0x8d8(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
 ...
 697:	90                   	nop
          s = "(null)";
 698:	ba d0 08 00 00       	mov    $0x8d0,%edx
        while(*s != 0){
...
free(void *ap)
{
 ...
 6b1:	a1 0c 0c 00 00       	mov    0xc0c,%eax
{
 ...
 6f1:	5b                   	pop    %ebx
  freep = p;
 6f2:	89 15 0c 0c 00 00    	mov    %edx,0xc0c
}
 6f8:	5e                   	pop    %esi
 6f9:	5f                   	pop    %edi
 6fa:	5d                   	pop    %ebp
 ...
 729:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 72c:	89 15 0c 0c 00 00    	mov    %edx,0xc0c
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 749:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 74c:	8b 3d 0c 0c 00 00    	mov    0xc0c,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 78f:	73 4f                	jae    7e0 <malloc+0xa0>
    ...
    if(p == freep)
 791:	8b 3d 0c 0c 00 00    	mov    0xc0c,%edi
 ...
 7ba:	e8 f1 fe ff ff       	call   6b0 <free>
  return freep;
 7bf:	8b 15 0c 0c 00 00    	mov    0xc0c,%edx
 ...
 7ec:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
 7ef:	89 15 0c 0c 00 00    	mov    %edx,0xc0c
}
 ...
 7ff:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
 800:	c7 05 0c 0c 00 00 10 	movl   $0xc10,0xc0c
 807:	0c 00 00 
    base.s.size = 0;
 80a:	bf 10 0c 00 00       	mov    $0xc10,%edi
    base.s.ptr = freep = prevp = &base;
 80f:	c7 05 10 0c 00 00 10 	movl   $0xc10,0xc10
 816:	0c 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 819:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 81b:	c7 05 14 0c 00 00 00 	movl   $0x0,0xc14
 822:	00 00 00 
    if(p->s.size >= nunits){
